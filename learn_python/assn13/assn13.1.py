...
Actual data: http://python-data.dr-chuck.net/comments_293764.xml (Sum ends with 25)
...
total = 0
url_data = raw_input("Enter location: ")
data = urllib.urlopen(url_data).read()
characters = len(data)
my_tree = ET.fromstring(data)
counts = my_tree.findall('.//count')

for each in counts:
    total += int(each.text)

print "Retrieving", url_data
print "Retrieved {} characters".format(characters)
print "Count:", total
